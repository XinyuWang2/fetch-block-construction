...
from fetch_block_construction.envs.robotics import rotations
...
class FetchBlockConstructionEnv(fetch_env.FetchEnv, gym_utils.EzPickle):
  ...
  def __init__(self,
    ...
    with tempfile.NamedTemporaryFile(
        mode='wt',
        dir=F"{os.path.dirname(os.path.dirname(os.path.realpath(__file__)))}/assets/fetch/",
        delete=False,
        suffix=".xml") as fp:
    ...
    gym_utils.EzPickle.__init__(self, initial_qpos, num_blocks, reward_type,
                                obs_type, render_size, stack_only)
  ...
  def gripper_pos_far_from_goals(self, achieved_goal, goal):
    ...
    block_goals = goal[
        ..., :-3]  # Get all the goals EXCEPT the zero'd out grip position

    distances = [
        np.linalg.norm(
            gripper_pos - block_goals[..., i * 3:(i + 1) * 3], axis=-1)
  ...
  def subgoal_distances(self, goal_a, goal_b):
    ...
    for i in range(self.num_blocks - 1):
      assert goal_a[..., i * 3:(i + 1) * 3].shape == goal_a[..., (i + 1) *
                                                            3:(i + 2) * 3].shape
    return [
        np.linalg.norm(
            goal_a[..., i * 3:(i + 1) * 3] - goal_b[..., i * 3:(i + 1) * 3],
            axis=-1) for i in range(self.num_blocks)
    ]
  ...
  def compute_reward(self, achieved_goal, goal, info):
    ...
    if self.reward_type == 'incremental':
      ...
      reward = -np.sum([(d > self.distance_threshold).astype(np.float32)
      ...
      np.putmask(reward, reward == 0,
                 self.gripper_pos_far_from_goals(achieved_goal, goal))
      return reward
    elif self.reward_type == 'block1only':
      return -(subgoal_distances[0] > self.distance_threshold).astype(
          np.float32)
      ...
      # Dense incremental
      stacked_reward = -np.sum([(d > self.distance_threshold).astype(np.float32)
                                for d in subgoal_distances],
                               axis=0)
      ...
      if stacked_reward != 0:
        ...
        block_goals = goal[..., :-3]
        reward -= .01 * np.linalg.norm(gripper_pos -
                                       block_goals[next_block_id * 3:
                                                   (next_block_id + 1) * 3])
      return reward
    else:
  ...
  def _render_callback(self):
    ...
    for i in range(self.num_blocks):
      ...
      self.sim.model.site_pos[site_id] = self.goal[i * 3:(i + 1) *
                                                   3] - sites_offset[i]
  ...
  def _sample_goal(self):
    ...
    if case == "Singletower":
      goal_object0 = self.initial_gripper_xpos[:3] + self.np_random.uniform(
          -self.target_range, self.target_range, size=3)
      goal_object0 += self.target_offset
      goal_object0[2] = self.height_offset

      if self.target_in_the_air and self.np_random.uniform(
      ) < 0.5 and not self.stack_only:
      ...
      for i in range(num_configured_blocks):
        if hasattr(self, "stack_only") and self.stack_only:
          ...
          objecti_xy = self.initial_gripper_xpos[:2] + self.np_random.uniform(
      ...
      def rot_matrix(A, B):
        ...
        R = np.identity(3) + skew(v) + np.dot(skew(v), skew(v)) * (
            (1 - c) / s**2)
        return R
      ...
      x_scaling = .06
      start_point = self.initial_gripper_xpos[:3] + self.np_random.uniform(
          -self.target_range, self.target_range, size=3)
      xs, zs = get_xs_zs(start_point)  # Just temporary
      # xs is actually ys, because we rotate

      attempt_count = 0
      while start_point[1] + max(
          xs) * x_scaling > self.initial_gripper_xpos[1] + self.target_range:
        start_point = self.initial_gripper_xpos[:3] + self.np_random.uniform(
            -self.target_range, self.target_range, size=3)
        if attempt_count > 10:
      ...
      for i in range(self.num_blocks):
        ...
        if i > 0:
          ...
          new_goal = rot_matrix(
              new_goal_vec, target_dir_vec) @ (new_goal - goals[0]) + goals[0]
      ...
      for i in range(num_towers):
        base_xy = self.initial_gripper_xpos[:2] + self.np_random.uniform(
            -self.target_range, self.target_range, size=2)
        while not np.all([
            np.linalg.norm(base_xy - other_xpos) >= 0.06
            for other_xpos in tower_bases
        ]):
          base_xy = self.initial_gripper_xpos[:2] + self.np_random.uniform(
              -self.target_range, self.target_range, size=2)
      ...
      for i in range(self.num_blocks - 1):
        objecti_xy = self.initial_gripper_xpos[:2] + self.np_random.uniform(
            -self.target_range, self.target_range, size=2)
        while not np.all(
            [np.linalg.norm(objecti_xy - goal[:2]) > .071 for goal in goals]):
          objecti_xy = self.initial_gripper_xpos[:2] + self.np_random.uniform(
              -self.target_range, self.target_range, size=2)
        goal_objecti = np.zeros(3)
